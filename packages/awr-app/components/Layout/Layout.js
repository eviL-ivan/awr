...
const ToggleSidebarButton = styled.div`
  ...
  left: ${p =>
    p.expanded
      ? `calc(${p.theme.sidebarExpanded.width} - 3px)`
      : `calc(${p.theme.sidebar.width} - 27px)`};
  ...
  background: ${p =>
    !p.expanded
      ...
      : p.theme.sidebar.background} !important;
  color: ${p =>
    p.expanded
      ? p.theme.palette.mainColor
      : p.theme.sidebar.background} !important;
  ...
  render() {
    ...
    return [
      <ToggleSidebarButton expanded={expanded}>
        <ExpandButton expanded={expanded} onClick={this.toggleSidebar}>
          <ArrowLeftIcon />
        </ExpandButton>
      </ToggleSidebarButton>,
